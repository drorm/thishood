...
public class NineBlockIdenticonRenderer implements IdenticonRenderer {
	...
	private static final byte[] patchTypes[] = {
			patch0, patch1, patch2, patch3, patch4, patch5, patch6, patch7, patch8, patch9, patch10, patch11, patch12, patch13, patch14, patch0
	};

	private static final byte patchFlags[] = {
			PATCH_SYMMETRIC, 0, 0, 0, PATCH_SYMMETRIC, 0, 0, 0, PATCH_SYMMETRIC, 0, 0, 0, 0, 0, 0, PATCH_SYMMETRIC + PATCH_INVERTED
	};
	...
	protected BufferedImage renderQuilt(int code, int size) {
		...
		// middle patch
		drawPatch(g, patchSize, patchSize, middleType, 0, middleInvert, fillColor, strokeColor);

		// side patchs, starting from top and moving clock-wise
		drawPatch(g, patchSize, 0, sideType, sideTurn++, sideInvert, fillColor, strokeColor);
		drawPatch(g, patchSize * 2, patchSize, sideType, sideTurn++, sideInvert, fillColor, strokeColor);
		drawPatch(g, patchSize, patchSize * 2, sideType, sideTurn++, sideInvert, fillColor, strokeColor);
		drawPatch(g, 0, patchSize, sideType, sideTurn++, sideInvert, fillColor, strokeColor);

		// corner patchs, starting from top left and moving clock-wise
		drawPatch(g, 0, 0, cornerType, cornerTurn++, cornerInvert, fillColor, strokeColor);
		drawPatch(g, patchSize * 2, 0, cornerType, cornerTurn++, cornerInvert, fillColor, strokeColor);
		drawPatch(g, patchSize * 2, patchSize * 2, cornerType, cornerTurn++, cornerInvert, fillColor, strokeColor);
		drawPatch(g, 0, patchSize * 2, cornerType, cornerTurn++, cornerInvert, fillColor, strokeColor);
