...
/**
 * Service responsible for rendering of templates.
 * Idea/code was got from 'grails-template-engine' plugin which didn't work (v0.2).
 * This implementation checks if it executed from Controller (request is available) and uses it attributes. Otherwise it creates a mock request to simulate the same behavior
 *
 * Major purpose of usage this service is email generation
 */
class TemplateRenderService {
    ...
    String render(templateName, model, pluginName = null) {
    ...
    protected String resolveViewUri(String viewName, HttpServletRequest request) {
        ...
        if (viewName.startsWith("/")) {
            ...
            if (!request) throw new IllegalArgumentException("View cannot be loaded from relative view paths where there is no current HTTP request")
